...
```bash
grep -rh "from '@/" src/app/ src/components/ | sed "s/.*from '/@/" | sed "s/'.*//" | sort -u
```
...
1. Read `src/app/api/auth/login/route.ts` (or the first non-trivial API route built in Phase 5).
...
1. Read 2–3 `page.tsx` files from `src/app/(authenticated)/` built in Phases 7–8.
